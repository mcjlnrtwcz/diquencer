...
class Sequence:
    def __init__(self, tempo=120, events=[]):
        ...
        if self._events:
            ...
            stop_event = StopEvent(
                ...
                + last_event.pattern.length * last_event.repetitions * 24 * 4
            )
    ...
    def from_raw_data(cls, raw_data, start_pattern_idx=0):
        ...
        for index, event in enumerate(patterns):
            pattern = Pattern(
                event["name"], event["pattern"], event["bank"], event["length"]
            )
            if index != 0:
                last_event_pulses += pattern.length * event["repetitions"] * 24 * 4
            ...
            mute_event = MuteEvent(
                0 if index == 0 else last_event_pulses - 1, event["playing_tracks"]
            )
    ...
    def patterns(self) -> List[Pattern]:
        return [
            event.pattern for event in self._events if isinstance(event, PatternEvent)
        ]

    @property
    def next_pattern(self) -> Union[Pattern, None]:
        pattern_it = filter(lambda event: isinstance(event, PatternEvent), self._events)
        try:
